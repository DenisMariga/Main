...
class Lowercurriculummark extends MY_Controller {

    public $data = array();

    function __construct() {
        parent::__construct();
        $this->load->model('Lowercurriculummark_Model', 'mark', true);   
       ...
       // check_permission(VIEW);
        ...
        if ($_POST) {
            ...
            if(!$school->academic_year_id){
                ...
                redirect('exam/lower_curriculum_mark/index');
            }
            ...
            if (!empty($this->data['students'])) {
                ...
                foreach ($this->data['students'] as $obj) {
                    ...
                    $mark = $this->mark->get_single('lower_marks', $condition);
                    ...
                    if (empty($mark)) {
                        ...
                        $this->mark->insert('lower_marks', $data);
                    }
                }
            }
        ...
        $this->layout->view('lower_curriculum_mark/index', $this->data);
    }
    ...
    * @return          : null 
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if(!$school->academic_year_id){
                error($this->lang->line('set_academic_year_for_school'));
                redirect('exam/lowercurriculummark/index');
            }
            ...
            if (!empty($_POST['students'])) {
                ...
                foreach ($_POST['students'] as $key => $value) {
                    ...
                    $data['obtain'] = $_POST['obtain'][$value];    
                    $data['lower_exam_total_mark'] = $_POST['lower_exam_total_mark'][$value];

                    $data['status'] = 1;
                    $data['created_at'] = date('Y-m-d H:i:s');
                    $data['created_by'] = logged_in_user_id();
                    $this->mark->update('lower_marks', $data, $condition);
                }
            }
        ...
        $this->layout->view('lowercurriculummark/index', $this->data);
    }
