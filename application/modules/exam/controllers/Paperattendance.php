 ...
 * @description     : Manage paperattendance for student who do a paper.  
...
class Paperattendance extends MY_Controller {

    public $data = array();

    function __construct() {
        parent::__construct();
        $this->load->model('Paperattendance_Model', 'attendance', true);        
    }
    ...
    * @type            : Function
    * @function name   : index
    * @description     : Load "paper attendance" user interface                 
    *                    and Process to manage Student exam paper attendance   
    * @param           : null
    * @return          : null 
    * ********************************************************** */ 
    public function index() {
        ...
        if ($_POST) {
            ...
            if(!$school->academic_year_id){
                ...
                redirect('exam/paper_attendance/index');
            }
            
            
            $this->data['students'] = $this->attendance->get_student_list($school_id, $exam_id, $class_id, $section_id, $subject_id, $paper_id,$school->academic_year_id);
            ...
            if (!empty($this->data['students'])) {
                ...
                foreach ($this->data['students'] as $obj) {
                    ...
                    $attendance = $this->attendance->get_single('paper_attendances', $condition);
                    ...
                    if (empty($attendance)) {
                        ...
                        $this->attendance->insert('paper_attendances', $data);
                    }
                }
            }
           ...
           $paper = $this->attendance->get_single('lp_paper_details', array('id'=>$paper_id));            
           create_log('Has been process paper attendance for : '.$subject->name. ', '. $class->name.', '. $paper->Ptitle);
        }
        ...
        $this->layout->title($this->lang->line('paper_attendance') . ' | ' . SMS);
        $this->layout->view('paper_attendance/index', $this->data);        
    }
    ...
    * ********************************************************** */ 
    public function update_single() {
        ...
        $condition['paper_id'] = $this->input->post('paper_id');
        ...
        if ($this->attendance->update('paper_attendances', $data, $condition)) {
            echo TRUE;
        } else {
            echo FALSE;
        }
